...
CREATE INDEX IF NOT EXISTS idx_telemetry_location ON telemetry(lat, lon);

-- Index for ignition-based queries (trip detection)
CREATE INDEX IF NOT EXISTS idx_telemetry_ignition ON telemetry(device_id, ignition, timestamp DESC);
...
CREATE INDEX IF NOT EXISTS idx_alerts_acknowledged ON alerts(tenant_id, acknowledged, created_at DESC);
...
CREATE INDEX IF NOT EXISTS idx_alerts_severity ON alerts(tenant_id, severity, acknowledged);
...
CREATE INDEX IF NOT EXISTS idx_telemetry_date ON telemetry(device_id, DATE(timestamp));

-- Index for speed-based queries (violations)
CREATE INDEX IF NOT EXISTS idx_telemetry_speed ON telemetry(device_id, speed, timestamp DESC);

-- Index for fuel level queries
CREATE INDEX IF NOT EXISTS idx_telemetry_fuel ON telemetry(device_id, fuel_level, timestamp DESC);
